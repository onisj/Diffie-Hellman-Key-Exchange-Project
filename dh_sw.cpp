...
)
{
  ...
  NN_DigitDivLH(aLow, t, c);  // This function implements the code shown below
  /*
...
)
{
  ...
  to_hw2.write(c);
  to_hw3.write(aLow);
  //Signal hardware to start
  hw_enable.write(true);

    //WAIT hw_done = false, wait fo hw_enable = true, go to EXECUTE
    while (true)
    {
      if (hw_done.read() == true) break;
      //cout <<  hw_enable << " going into HARDWARE COMPUTING\n";
      wait();
    }
    //EXECUTE
      //Read HW output and notify to stop
       aLow = from_hw.read();
      //notify hardware to stop
      hw_enable.write(false);  
    
    //FINISH -wait for hardware to complete
    while (true)
    {
